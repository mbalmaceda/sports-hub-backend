...
func (r *UserRepository) FindByID(ctx context.Context, id string) (*user.User, error) {
	const q = `
		SELECT id, name, email, role, COALESCE(push_token,''), password_hash, created_at, updated_at
		FROM users WHERE id = $1`
	u := &user.User{}
	err := r.pool.QueryRow(ctx, q, id).
		Scan(&u.ID, &u.Name, &u.Email, &u.Role, &u.PushToken, &u.PasswordHash, &u.CreatedAt, &u.UpdatedAt)
...
func (r *UserRepository) Create(ctx context.Context, u *user.User) error {
	...
	err := r.pool.QueryRow(ctx, q, u.Name, u.Email, u.Role, u.PasswordHash).
